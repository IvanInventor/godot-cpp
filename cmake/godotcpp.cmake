...
# Equivalent to scons' `GodotCPPDocData` builder
function(godot_make_doc)
  find_package(Python3 3.4 REQUIRED)
  set(options)
  set(oneValueArgs DESTINATION COMPRESSION)
  set(multiValueArgs SOURCES)
  cmake_parse_arguments(MAKE_DOC "${options}" "${oneValueArgs}"
                        "${multiValueArgs}" ${ARGN})

  if("${MAKE_DOC_COMPRESSION}" STREQUAL "")
    set(make_doc_compression "Z_BEST_COMPRESSION")
  else()
    set(make_doc_compression ${MAKE_DOC_COMPRESSION})
  endif()

  add_custom_command(
    OUTPUT ${MAKE_DOC_DESTINATION}
    COMMAND
      "${Python3_EXECUTABLE}"
      "${CMAKE_CURRENT_FUNCTION_LIST_DIR}/../docs_generator.py"
      "${make_doc_compression}" "${MAKE_DOC_DESTINATION}" ${MAKE_DOC_SOURCES}
    VERBATIM
...
set(GODOT_CONFIGS_WITH_DEBUG
    "Debug;RelWithDebInfo"
    CACHE STRING "Configurations that should have debug symbols\
 (Modify if support for custom configurations is needed)")
...
elseif(CMAKE_SYSTEM_NAME STREQUAL "Emscripten") # Set by providing Emscripten
                                                # toolchain
  set(DEFAULT_GODOT_PLATFORM "web")
elseif(CMAKE_SYSTEM_NAME STREQUAL "Android") # Set by providing Android
                                             # toolchain
...
set(GODOT_PLATFORM
    "${DEFAULT_GODOT_PLATFORM}"
    CACHE STRING "[Auto-detected] Target platform\
 (linux, macos, windows, android, ios, web)")

if("${GODOT_PLATFORM}" STREQUAL "NOTFOUND")
  message(
    FATAL_ERROR "Could not auto-detect platform for \"${CMAKE_SYSTEM_NAME}\"\
 automatically, please specify with -DGODOT_PLATFORM=<platform>")
endif()

message(STATUS "Platform detected: ${GODOT_PLATFORM}")

set(GODOT_GDEXTENSION_DIR
    "${CMAKE_CURRENT_SOURCE_DIR}/gdextension"
    CACHE PATH "Path to a directory containing GDExtension interface header")

set(GODOT_CUSTOM_API_FILE
    "${GODOT_GDEXTENSION_DIR}/extension_api.json"
    CACHE FILEPATH "Path to GDExtension API JSON file")

set(GODOT_PRECISION
    "single"
    CACHE STRING "Floating-point precision level (single, double)")

set(GODOT_OPTIMIZE
    "auto"
    CACHE STRING "The desired optimization flags\
 (none, custom, debug, speed, speed_trace, size)")

set(GODOT_SYMBOLS_VISIBILITY
    "hidden"
    CACHE STRING
          "Symbols visibility on GNU platforms (default, visible, hidden)")

set(GODOT_BUILD_PROFILE
    ""
    CACHE FILEPATH "Path to a file containing a feature build profile")
...
option(GODOT_USE_HOT_RELOAD
       "Enable the extra accounting required to support hot reload"
       ${DEFAULT_GODOT_USE_HOT_RELOAD})
...
option(GODOT_SYSTEM_HEADERS
       "Mark the header files as SYSTEM. This may be useful to suppress\
 warnings in projects including this one" OFF)
...
set(GODOT_WARNING_AS_ERROR
    "${DEFAULT_WARNING_AS_ERROR}"
...
# Configuration of build targets: * Editor or template * Debug features
# (DEBUG_ENABLED code) * Dev only code (DEV_ENABLED code) * Optimization level *
# Debug symbols for crash traces / debuggers Keep this configuration in sync
# with SConstruct in upstream Godot.
list(
  APPEND
  GODOT_DEFINITIONS
  $<$<BOOL:${GODOT_THREADS}>:
  ...
  # DEBUG_ENABLED enables debugging *features* and debug-only code, which is
  # intended to give *users* extra debugging information for their game
  # development.
  DEBUG_ENABLED
  ...
  # DEV_ENABLED enables *engine developer* code which should only be compiled
  # for those working on the engine itself.
  DEV_ENABLED
  >
...
# Suffix See more prefix appends in platform-specific configs
...
if((NOT "${GODOT_PLATFORM}" STREQUAL "macos") AND (NOT "${GODOT_PLATFORM}"
                                                   STREQUAL "ios"))
...
# file(GENERATE OUTPUT "${CMAKE_CURRENT_BINARY_DIR}/flags-${CONFIG}.txt" CONTENT
# "C_FLAGS '${GODOT_C_FLAGS}'\nCXX_FLAGS '${GODOT_CXX_FLAGS}'\nLINK_FLAGS
# '${GODOT_LINK_FLAGS}'\nCOMPILE_WARNING_FLAGS
# '${GODOT_COMPILE_WARNING_FLAGS}'\nDEFINITIONS '${GODOT_DEFINITIONS}'" TARGET
# ${PROJECT_NAME} )
